...
func apply_damage(data: DamageData):
...
func handle_actions_when_on_floor() -> void:
	...
	var can_roll = not_roll_and_attack and !is_crouching and !is_jumping and PlayerStats.can_roll()
	var can_attack = not_roll_and_attack and PlayerStats.can_attack()
	...
	if can_roll and Input.is_action_just_pressed("roll"):
		...
		#PlayerStats.update_energy(-PlayerStats.energy_cost_to_roll)
	if can_attack and Input.is_action_just_pressed("attack"):
		is_attacking = true
		handle_attack_combo_count()
		
	#update_colision_shape_when_crouch()
func handle_actions_when_on_air() -> void:
	...
	if can_attack and Input.get_action_strength("attack"):
		is_attacking = true
		handle_attack_combo_count()
...
func _on_level_up(level: int):
	var float_label: FloatLabel = preload("res://src/gui/float_label.tscn").instantiate()
