...
@export var attached_gems: Dictionary[int, GemItem] = {}
...
func clone() -> EquipmentItem:
	...
	copy.attached_gems = self.attached_gems
...
func setup(enemy_stats: EnemyStats) -> void:
	...
	self.spawn_chance = _calculate_spawn_chance()
	self.item_category = Item.CATEGORY.EQUIPMENTS
	self.equipment_group = _determine_equipment_group()
	self.is_unique = (equipment_group == GROUPS.UNIQUES)
	self.equipment_set = _determine_equipment_set(enemy_stats.level)
	self.item_rarity = _calculate_item_rarity(enemy_stats)
	self.available_sockets = clampi(item_rarity, 0, 4) # Itens mais raros podem ter mais slots de gemas
	self.equipment_type = _determine_equipment_type()
	self.item_subcategory = _determine_equipment_subcategory()

	# GERA PRIMEIRO OS ATRIBUTOS BASE (dano/defesa)
	_setup_base_values()

	# DEPOIS GERA OS ATRIBUTOS ADICIONAIS
	self.item_attributes.append_array(_generate_attributes())
	self.item_name = _generate_name()
	self.item_descriptions = _generate_equipment_descriptions()
	self.item_id = _generate_equipment_id()
	self.item_price = _calculate_item_price(_calculate_equipment_base_price())
	self.equipment_power = _calculate_equipment_power()
	_setup_texture()


func _calculate_spawn_chance() -> float:
...
func _determine_equipment_type() -> TYPE:
...
func _determine_equipment_subcategory() -> Item.SUBCATEGORY:
...
func _filter_equipments_sets_config(_config: SetLevelConfig) -> bool:
...
func _calculate_item_rarity(_enemy_stats: EnemyStats) -> Item.RARITY:
...
func _generate_attributes() -> Array[ItemAttribute]:
	...
	for i in range(num_attributes):
		...
		var base_value = _calculate_attribute_value(random_type)
...
func _generate_equipment_id() -> String:
...
func _setup_base_values() -> void:
	if self.item_subcategory == Item.SUBCATEGORY.WEAPON:
		_generate_damage_stats()
	else:
		_generate_defense_stats()


func _generate_damage_stats() -> void:
...
func _generate_defense_stats() -> void:
...
func _calculate_equipment_base_price() -> int:
...
func get_all_attributes() -> Array[ItemAttribute]:
	...
	if damage != null and damage.value > 0:
		all_attributes.append(damage)
	if defense != null and defense.value > 0:
		all_attributes.append(defense)

	for slot_index in attached_gems:
		var gem = attached_gems[slot_index]
		if gem:
			all_attributes.append_array(gem.item_attributes)
...
func _calculate_equipment_power() -> float:
	var power: float = 0.0
	
	# 1. Pontuação base por raridade (maior peso)
	var rarity_points = _calculate_rarity_points()
	power += rarity_points
	
	# 2. Bônus por ser item único
	var unique_bonus = _calculate_unique_bonus()
	power += unique_bonus
	
	# 3. Pontos por nível do item
	var level_points = _calculate_level_points()
	power += level_points
	
	# 4. Pontos por atributos (quantidade e qualidade)
	var attributes_points = _calculate_attributes_points()
	power += attributes_points
	
	# 5. Pontos por dano/defesa base (se aplicável)
	var base_stats_points = _calculate_base_stats_points()
	power += base_stats_points
	
	# 6. Pontos por gemas anexadas
	var gems_points = _calculate_gems_points()
	power += gems_points
	
	return max(0, power)

func _calculate_rarity_points() -> float:
...
func _calculate_unique_bonus() -> float:
	if is_unique:
		# Bônus significativo para itens únicos (equivalente a ~1 nível de raridade extra)
		return _calculate_rarity_points() * 0.4
	return 0.0

func _calculate_level_points() -> float:
...
func _calculate_attributes_points() -> float:
...
func _calculate_base_stats_points() -> float:
	var points: float = 0.0
...
func _calculate_gems_points() -> float:
	var points: float = 0.0
	
	for slot_index in attached_gems:
		var gem = attached_gems[slot_index]
		if gem:
			# Calcula o poder base da gema (atributo principal)
			var gem_power = 0.0
			for attribute in gem.item_attributes:
				...
				if value_range > 0:
					var percentage = (attribute.value - min_value) / value_range
					# Pontuação base para gemas com multiplicador
					gem_power += 20.0 * percentage * EquipmentConsts.GEM_POWER_MULTIPLIER
			
			# Bônus por qualidade da gema
			var quality_bonus = 1.0
			match gem.gem_quality:
				GemItem.QUALITY.PRISTINE:
					quality_bonus = 2.0
				GemItem.QUALITY.EXQUISITE:
					quality_bonus = 1.8
				GemItem.QUALITY.FLAWLESS:
					quality_bonus = 1.6
				GemItem.QUALITY.REFINED:
					quality_bonus = 1.4
				GemItem.QUALITY.COMMON:
					quality_bonus = 1.2
				GemItem.QUALITY.FRAGMENTED:
					quality_bonus = 1.0
			
			# Bônus adicional para gemas únicas
			if gem.is_unique:
				quality_bonus *= 1.5
			
			points += gem_power * quality_bonus
