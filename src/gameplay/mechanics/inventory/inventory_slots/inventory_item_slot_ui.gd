...
func _try_move_item(target_slot: BaseSlotUI) -> void:
	if target_slot.is_in_group(InventoryItemSlotUI.GROUP_NAME):
		...
		_try_move_to_equippemt_slots(target_slot)
...
func _try_move_to_equippemt_slots(equipment_slot: EquipmentItemSlotUI) -> void:
	# Se o item movido é um Equipamento
	if current_item is EquipmentItem:
		var inventory_item = current_item as EquipmentItem
		...
		if inventory_item.equipment_type == equipment_slot.slot_type:
			...
			if equipment_slot.current_item is EquipmentItem:
				...
				PlayerEquipments.equip(inventory_item)
				InventoryManager.item_drag_ended.emit(true)
