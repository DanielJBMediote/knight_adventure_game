...
const PANEL_MIN_SIZE = Vector2(420, 460)
...
@onready var description: Label = $Panel/MarginContainer/ScrollContainer/ContentContainer/Content/Description
...
@onready var equip_sockets_slots: EquipmentSocketSlots = $Panel/MarginContainer/ScrollContainer/ContentContainer/Content/EquipSocketsSlots
...
func update_bonus_attributes_info(equipment: EquipmentItem) -> void:
	pass
	# var attributes = PlayerEquipments.get_all_equipped_items()
	# var label: Label = attribute_bonus_container.get_node("Label")
	# var attribute_label: Label = attribute_bonus_container.get_node("AttributeLabel")

	# if equipment.equipment_group == EquipmentItem.GROUPS.COMMON:
	...
	# 		var attribute_name = LocalizationManager.get_ui_text(ItemAttribute.ATTRIBUTE_KEYS[attribute.type])
...
func update_gem_sockets_info(equipment: EquipmentItem) -> void:
	...
	for slot_index in equipment.available_sockets:
		if slot_index < gems.size():
			...
			if gem:
				equip_sockets_slots.gems.append(gem)
			else:
				equip_sockets_slots.gems.append(null)
		else:
			equip_sockets_slots.gems.append(null)
	equip_sockets_slots.setup()

func update_consumables_informations(_item: Item) -> void:
	base_stats_container.hide()
	trash_item_button.show()
	equip_sockets_slots.hide()
...
func update_item_attributes_info(item: Item) -> void:
	...
	if attributes.is_empty():
		...
		attribute_label.label_color = LocalizationManager.get_ui_text("no_attributes")
	...
	for attribute in attributes:
		...
		if is_equipments:
			...
			main_attribute_label.label_color = attribute_color
		
		main_attribute_label.value_text = str("+", formatted_value, " ", attribute_name)
		...
		if showing_advanced and is_equipments:
			...
			advanced_label.value_text = str("(", min_formatted, " - ", max_formatted, ")")
			advanced_label.label_color = Color.SLATE_GRAY
