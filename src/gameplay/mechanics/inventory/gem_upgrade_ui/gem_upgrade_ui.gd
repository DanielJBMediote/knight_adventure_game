...
@onready var req_content_ui_scene: PackedScene = preload(
	"res://src/gameplay/mechanics/inventory/gem_upgrade_ui/requeriment_content_ui.tscn"
)
...
func update_requeriments() -> void:
	for child in requeriment_items.get_children():
		child.queue_free()
	...
	if rune:
		...
		rune_requeriment_ui.quantity_needed = current_gem.get_number_of_runes_to_upgrade() * amount_gem_to_upgrade
	...
	gem_requeriment_ui.quantity_needed = 1 * amount_gem_to_upgrade
	gem_requeriment_ui.quanity = max(amount_gems_in_inventory - 1, 0)
	gem_requeriment_ui.setup(current_gem)
