...
func remove_items(items_to_remove: Array[Item], total_amount: int) -> bool:
	...
	if remaining_amount > 0:
		var all_items_of_type = find_many_items_by_id(
			items_to_remove[0].item_id if items_to_remove.size() > 0 else ""
		)
...
func swap_items(from_slot: int, to_slot: int) -> void:
	...
	# Caso 2: Mesmo item e stackable - tenta juntar stacks
		...
		from_item.item_id == to_item.item_id
		and from_item.item_rarity == to_item.item_rarity
		and from_item.stackable
		and to_item.stackable
	):
